...
class EventStore:
    ...
    async def append(
        ...
        Position convention: first event in a stream = position 1
        expected_version: -1 for new stream, 0 for empty stream, N for stream with N events
        """
        async with self._pool.acquire() as conn:
            async with conn.transaction():
                ...
                if row is None:
                    ...
                    current = 0  # Position 0 means "no events yet"
                else:
                    current = row["current_version"]
                    # For existing streams, expected_version should match current version
                    # current=0 means stream has events at positions 1..current
                ...
                # 4. Insert each event (positions are 1-indexed)
                positions = []
                meta = {**(metadata or {})}
                if causation_id: meta["causation_id"] = causation_id
                
                # Current is the number of events already in stream (0-indexed count)
                # Next position should be current + 1 (1-indexed)
                ...
                # 5. Update stream version (final position = last event position)
                final_version = next_position + len(events) - 1
                await conn.execute(
                    "UPDATE event_streams SET current_version=$1 WHERE stream_id=$2",
                    final_version, stream_id)
                return positions
    ...
    async def load_stream(
        ...
        async with self._pool.acquire() as conn:
            ...
            events = []
            for row in rows:
                # asyncpg already deserializes JSONB to dict
                e = dict(row)
